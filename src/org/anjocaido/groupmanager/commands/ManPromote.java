 ...
 *
 */
public class ManPromote extends BaseCommand {
	...
	protected boolean parseCommand(@NotNull String[] args) {
		...
		if (args.length != 2) {
			sender.sendMessage(ChatColor.RED + Messages.getString("ERROR_REVIEW_ARGUMENTS") + Messages.getString("MANPROMOTE_SYNTAX")); //$NON-NLS-1$ //$NON-NLS-2$
			return true;
		}
		...
		if (!permissionHandler.hasGroupInInheritance(auxGroup, auxUser.getGroupName())) {
			sender.sendMessage(ChatColor.RED + Messages.getString("ERROR_NEW_GROUP_NOT_HIGHER")); //$NON-NLS-1$
			return true;
		}
		// Seems OK
